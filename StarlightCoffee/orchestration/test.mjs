...
describe("StarlightCoffeeShield", async function () {
	...
	// eslint-disable-next-line func-names
	describe("addPoints", async function () {
		this.timeout(3660000);
		try {
			await web3.connect();
		} catch (err) {
		...
		describe("First call", async function () {
			this.timeout(3660000);
			it("should call addPoints", async () => {
				try {
			...
			it("should update the merkle tree", async () => {
				try {
					...
					const path = await getSiblingPath("StarlightCoffeeShield", leafIndex);
	...
	// eslint-disable-next-line func-names
	describe("spendPoints", async function () {
		this.timeout(3660000);
		try {
			await web3.connect();
		} catch (err) {
		...
		describe("First call", async function () {
			this.timeout(3660000);
			it("should call spendPoints", async () => {
				try {
					// this starts up the merkle tree's event filter
					await startEventFilter("StarlightCoffeeShield");
					// this calls your function! It returns the tx from the shield contract
